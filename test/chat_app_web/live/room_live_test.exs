defmodule ChatAppWeb.RoomLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all rooms", %{conn: conn, room: room} do
      {:ok, _index_live, html} = conn |> log_in_user(user_fixture()) |> live(~p"/rooms")
    ...
    test "saves new room", %{conn: conn} do
      {:ok, index_live, _html} = conn |> log_in_user(user_fixture()) |> live(~p"/rooms")
  ...
  describe "Show" do
    ...
    test "displays room", %{conn: conn, room: room} do
      {:ok, _show_live, html} = conn |> log_in_user(user_fixture()) |> live(~p"/rooms/#{room}")
    ...
    test "updates room within modal", %{conn: conn, room: room} do
      {:ok, show_live, _html} = conn |> log_in_user(user_fixture()) |> live(~p"/rooms/#{room}")
